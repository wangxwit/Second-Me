...
**实现机制**: 见 [7.6 情感陪伴与反思机制](#76-情感陪伴与反思机制-emotional-companion--reflection)
...
**实现机制**: 见 [7.7 创意写作协同机制](#77-创意写作协同机制-creative-writing-collaboration)
...
**输入**: 原始数据（文本、图片、音频、文档文件）+ 用户传记信息（Bio）

**支持的文件类型**:
- **图片**: JPEG, PNG, GIF, WebP 等（通过 Vision 模型处理）
- **音频**: MP3, WAV, M4A 等（通过 Audio 模型处理）
- **文档**: PDF, DOCX, TXT, MD 等（通过文本处理器处理）
